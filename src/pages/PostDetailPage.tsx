    ...
    <div className="container mx-auto px-2 sm:px-4 py-8 space-y-10">
      ...
      <Card className="p-8 space-y-4 bg-card rounded-2xl shadow-lg border border-border">
        ...
        <p className="text-base text-accent">
          ...
          <Link to={`/users/${user.id}`} className="underline hover:text-primary transition">
            {user.name}
          </Link>
        </p>
      </Card>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {comments.map((comment) => (
            <Card key={comment.id} className="p-6 rounded-xl shadow border border-border bg-card group transition-transform hover:scale-[1.03] hover:shadow-lg">
              <h6 className="text-lg font-semibold text-primary flex items-center gap-2 mb-1">🗨️ {comment.name}</h6>
              <p className="text-sm text-muted-foreground mb-1">{comment.email}</p>
