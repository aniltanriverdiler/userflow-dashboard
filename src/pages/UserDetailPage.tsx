...
import type { ApiUser, ApiPost, ApiAlbum, ApiTodo } from "../types/types";

function UserDetailPage() {
  const user = useLoaderData() as ApiUser;
  const { user: currentUser } = useAuthStore();
  const userKey = currentUser?.id || "";
  const favoritePostsFromStore = useFavoritesStore(
    (state) => state.postsByUser[userKey]
  );
  const favoritePosts = favoritePostsFromStore ?? [];
  const addPost = useFavoritesStore((state) => state.addPost);
  const removePost = useFavoritesStore((state) => state.removePost);
  const [activeTab, setActiveTab] = useState("posts");
  const [tabData, setTabData] = useState<(ApiPost | ApiAlbum | ApiTodo)[]>([]);
    ...
    <div className="container mx-auto px-2 sm:px-4 py-6 space-y-8">
        ...
        <Card className="p-7 space-y-4 bg-card rounded-2xl shadow-lg border border-border">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold text-white shadow border-4 border-background bg-primary/90`}
              >
                {user.name[0]}
              </div>
              <div>
      ...
      <Tabs value={activeTab} onValueChange={handleSelect} className="w-full">
        <TabsList className="flex justify-center items-center gap-1 bg-muted rounded-xl shadow px-1 py-0.5 mx-auto w-fit">
          <TabsTrigger
            value="posts"
            className="flex items-center gap-1 text-sm font-semibold text-primary px-3 py-1.5 rounded-md data-[state=active]:bg-background data-[state=active]:text-accent transition-all"
          >
            📝 Posts
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="posts">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
              {tabData
                .filter((item): item is ApiPost => "title" in item && "userId" in item)
                .map((post) => {
                  const typedPost = post as ApiPost;
                  const isFav = favoritePosts.some((p) => p.id === post.id);
                  const toggle = () => {
                    if (isFav) removePost(post.id, currentUser?.id || "");
                    else {
                      addPost(
                        {
                          ...typedPost,
                  ...
                  return (
                    <Card
                      key={post.id}
                      className="p-5 space-y-2 h-full rounded-xl shadow border border-border bg-card group transition-transform hover:scale-[1.03] hover:shadow-lg"
                    >
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={toggle}
                          className={`rounded-full text-2xl transition-all duration-200 ${
                            isFav
        ...
        <TabsContent value="albums">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
              {tabData
                .filter((item): item is ApiAlbum => "title" in item && !("completed" in item))
                .map((album) => (
                  <Card
                    key={album.id}
                    className="p-4 rounded-xl shadow border border-border bg-card group transition-transform hover:scale-[1.03] hover:shadow-lg"
                  >
        ...
        <TabsContent value="todos">
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
              {tabData
                .filter((item): item is ApiTodo => "completed" in item)
                .map((todo) => (
                  <Card
                    key={todo.id}
                    className={`p-4 border-l-4 ${
                      todo.completed ? "border-accent" : "border-secondary"
                  ...
                  >
                    ...
                    <p className="text-sm">
                      ...
                      <span
                        className={
                          todo.completed ? "text-accent" : "text-secondary"
                        }
                      >
